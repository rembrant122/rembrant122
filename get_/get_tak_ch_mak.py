                        ...
                        if sum_ >= sum1:
                            ...
                            for item_sell in res_sell:
                                if float(item_sell['price']) == min(b_sell):
                                    ...
                                    sql.execute(
                                        f"INSERT INTO takers_change_makers VALUES ('{money}' , '{item['exchange']}|{item['paytype']}', '{i}' , '{sum}', '{get_crypto}', '{get}', '{sum1}', '{item_sell['exchange']}|{item_sell['paytype']}','{procent}%')")
                                    db.commit()
                            break
